...
with c2:
    ...
    <p>💼 <b>LinkedIn:</b> <a href="#" style="color:#ffff00;">Utku Çimen</a></p>
    </div>""", unsafe_allow_html=True)

# --- TEKNİK REHBER (SENSÖR BÖLÜMÜ) ---
st.header("📡 Teknik Rehber")
t1, t2, t3, t4 = st.tabs(["🧲 İndüktif Sensör", "🔮 Kapasitif Sensör", "👁️ Optik / Fotosel", "⚡ Ohm Yasası"])
...
[Image of Ohm's Law triangle formula]
    ...
    with col_calc1:
        v_val = st.number_input("Gerilim (Volt)", value=220.0, step=1.0)
        r_val = st.number_input("Direnç (Ohm)", value=10.0, step=1.0)
        if r_val > 0:
            i_res = v_val / r_val
            st.markdown(f'<p class="sensor-text">Hesaplanan Akım: {i_res:.2f} Amper</p>', unsafe_allow_html=True)
        else:
            st.error("Direnç 0 olamaz!")
    with col_calc2:
        st.markdown("""<div class="info-box"><b>Formül Hatırlatıcı:</b><br>
        V = Gerilim (Volt)<br>
        I = Akım (Amper)<br>
        R = Direnç (Ohm)</div>""", unsafe_allow_html=True)
